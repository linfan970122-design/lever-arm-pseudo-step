#!/usr/bin/env python3
"""M8b: the two recorded 6 September events, measurement-side gate arms.

Same quantities as analysis/m8/natural_events.py (step_gps, diff_rtk), plus what the
measurement gate did on those frames.  Writes data/m8b_measurement_gate/natural_events.csv.
"""
import csv
import os
...
EVENTS = [('E_-67.5deg', 1788681423.094, 0.4014, -67.54),
          ('E_-167.5deg', 1788680085.536, 0.7264, -167.51)]
GROUPS = {'E_-67.5deg': 'nat1', 'E_-167.5deg': 'nat2'}
ARMS = ['meas_q060', 'meas_q1e4', 'both_q060']
SLAB = {0: 'accept', 1: 'hold', 2: 'nopub'}
...
def main():
    ...
    for lbl, t0, rec_step, rec_dpsi in EVENTS:
        for arm in ARMS:
            tag = '%s_%s' % (GROUPS[lbl], arm)
            d = os.path.join(RUNS, tag)
            if not os.path.isfile(os.path.join(d, 'odom_gps.csv')):
                continue
            ...
            mg = os.path.join(d, 'meas_gate.csv')
            acts = []
            if os.path.isfile(mg):
                a = np.genfromtxt(mg, delimiter=',', names=True)
                m = (a['t'] >= t0 - 0.5) & (a['t'] < t0 + 10.0)
                for s in a['state'][m].astype(int):
                    if s and (not acts or acts[-1] != SLAB[s]):
                        acts.append(SLAB[s])
            rows.append(dict(event=lbl, arm=arm, rec_step_m=rec_step, rec_dpsi_deg=rec_dpsi,
                             ...
                             node_gate_actions=';'.join(kinds) if kinds else '-',
                             meas_gate_actions=';'.join(acts) if acts else '-'))
    if not rows:
